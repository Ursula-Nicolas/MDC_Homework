...
After filtering the data, the data set is reduced to `r nrow(cardio_data)`, therefore, `r nrow(data)-nrow(cardio_data)` cases have been removed. Now, the distribution of these variables looks as follows (Figure \ref{FigNoOutliers}):
...
```{r distribution, fig.cap= "Number of cases per cardiovascular group", fig.height= 2, fig.width= 2, echo= FALSE}
...
Furthermore, we want to see if all data is complete. There are `r sum(is.na(cardio_data))` missing cases. Therefore, the data is hereby ready to be used for analysis.
...
```{r plot_BMI_ap_hi, message = FALSE , fig.cap= "Scatter plot showing the relationship between BMI and systolic blood pressure\\label{BMI_ap_hi}", echo= FALSE}
...
```
The results show, that the empiric correlation coefficient is `r round(cor_BMI_aphi$estimate,2)`, which lies within the 95% confidence interval of `r round(cor_BMI_aphi$conf.int[1],2)` and `r round(cor_BMI_aphi$conf.int[2],2)`, while 0 is not included in the confidence interval. The correlation coefficient of `r round(cor_BMI_aphi$estimate,2)` shows a positive relationship between BMI and systolic blood pressure, however, is under 0.3, indicating, that the systolic blood pressure and the BMI have only a moderate linear relationship (see also @Cohen1988). Based on $\alpha = 0.05$ the relationship between both variables is significant as *p* `r format.pval(cor_BMI_aphi$p.value)`. 
...
The correlation coefficient between diastolic blood pressure and BMI differs between people with (`r round(cor_BMI_aplo_1$estimate,2)`) and without (`r round(cor_BMI_aplo_0$estimate,2)`) cardiovascular diseases seems to differ again (p-value under $\alpha = 0.05$), while people with cardiovascular diseases have a again a smaller correlation coefficient. Though, both correlation coefficients are again categorized between 0.1 and 0.3 [@Cohen1988].
...
There seems to be only a moderate relationship between blood pressure and BMI. Although correlation coefficients do differ between cardio groups (without disease > with disease) according to the Pearson-moment correlation, all correlation coefficients stay within the range of 0.1-0.3, therefore, in the same category according to Cohen [-@Cohen1988]. It has to be noted, that diastolic blood pressure versus BMI of the affected group while applying a 95% threshold, resulted even in a correlation coefficient lower than 0.1 (`r round(cor_BMI_aplo_1_95D$estimate,2)`).


The following table (Table \ref{tab:SummaryTableBloodBMI}) summarizes the findings of this relationship. Also, including a threshold of 95% quantile does not change anything regarding the relation of blood pressure and BMI. In general, however, the correlation coefficients decrease slightly for every comparison. This can be explained by 
...
cardio_data %>% group_by(cardio) %>%
  ...
  kable(col.names = c("Cardio Group", "mean", "SD", "max", "min"), 
        digits = 1, 
        caption = "Overview of age distribution for each cardio group", 
        booktabs = T) %>% 
  kable_styling(latex_options = c("striped","hold_position"))%>%  
  column_spec(2:4, width = "0.5in")%>% 
  add_footnote(c("Cardio group 0 = healthy, 1 = affected", "SD = standard deviation"), 
...
Furthermore, the average age difference between both groups is `r abs(mean(cardio_0$age)- mean(cardio_1$age)) %>% round(.,2)`, the population with cardiovascular diseases being older than the healthy population. The boxplot and density curve of Figure \ref{ageDis} strengthen this hypothesis.
...
While age seems to be slightly different distributed between cardio groups, gender looks fairly equally distributed between healthy and affected persons (Figure \ref{ageDis2})
...
dens_gender1 <- cardio_data %>% ggplot(.) + geom_density(aes(age, fill = gender), alpha = 0.5) + theme_bw() + theme(axis.text.y = element_blank(), 
...
Taking, furthermore, the different types of glucose into account a slight different distribution between group 1-2 and group 3 can be visually detected (Figure \ref{ageDis3}), while especially older people seem to have a glucose level well above the normal level.

```{r ageDistributionCardioGenderGluc, fig.height= 2, fig.cap= "Age distribution for cardio, gender and glucose\\label{ageDis3}", echo= FALSE}
cardio_data$gluc <- as.factor(cardio_data$gluc)
dens_glucose1 <- cardio_data %>% ggplot(.) + geom_density(aes(age, fill = gluc), alpha = 0.5) + theme_bw()+ theme(axis.text.y = element_blank(), 
                                                                                                                    axis.title.y = element_blank())
...
Further risk factors, which might trigger cardiovascular diseases may be smoking, alcohol abuse or insufficient physical activity. Table \ref{tab:RiskFactors} summarizes the distribution of these variables between the two types of *cardio*. To quantify the differences between both groups of cardio, a Chi-Square test will be undertaken for each risk factor.
...
CardioGroup <- c("Cardio 0","Cardio 1","Chi-Square")
...
kable(cbind(CardioGroup,RF1_per,RF2_per,RF3_per),
      col.names = c("Data","Smoking","Alcohol", "Physical activity"),
      caption = "Relative frequency of risk factors in percent",
      booktabs = T) %>% 
  kable_styling(latex_options = c("striped","hold_position"))
```
