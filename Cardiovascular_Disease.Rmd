...
## 2.5 Summary relationship bwetween blood pressure and BMI (\textcolor{red}{Tasks 2-4})
...
Taking, furthermore, the different types of glucose into account a slight different age distribution between glucose category 1-2 and category 3 can be visually detected (Figure \ref{ageDis3}), while especially older people seem to have a glucose level well above the normal level (glucose 3).
...
Further risk factors, which might trigger cardiovascular diseases may be smoking, alcohol abuse or insufficient physical activity. Table \ref{tab:RiskFactors} summarizes the distribution of these variables between the two types of *cardio*. To quantify the differences between both groups of cardio, a $\chi$^2^ test will be undertaken for each risk factor.
